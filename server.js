...
app.get('/profile', (req, res) => {
    ...
    db.serialize(() => {
        db.get("SELECT users.id_user, users.username, users.birthdate, users.email, stats.squadra, stats.percentuale_tiro_3_punti FROM users  JOIN stats ON users.id_user = stats.id_user WHERE users.username = ?", [username], (err, row) => {
            if (err) {
                console.error(err.message);
